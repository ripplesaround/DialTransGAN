...
class TranGenerator(nn.Module):
    def __init__(self,embedding_dim,vocab_size, max_seq_len, padding_idx, gpu=False):
        super(TranGenerator, self).__init__()
        self.name = "Trans-based"
        ...
        # self.lstm = nn.LSTM(embedding_dim, hidden_dim, batch_first=True)
        self.trans2out = nn.Linear(embedding_dim, vocab_size)
        self.softmax = nn.LogSoftmax(dim=-1)

        self.init_params()

    def forward(self, inp, hidden = 0):
        """
        Embeds input and applies transformer
        :param inp: batch_size * seq_len
        """
        emb = self.embeddings(inp)  # batch_size * len * embedding_dim
        if len(inp.size()) == 1:
        ...
        out = out.contiguous().view(-1, self.embedding_dim)  # out: (batch_size * len) * hidden_dim
        out = self.trans2out(out)  # (batch_size * seq_len) * vocab_size
        # out = self.temperature * out  # temperature
        pred = self.softmax(out)
    ...
    def init_hidden(self, batch_size=cfg.batch_size):
        h = torch.zeros(1, batch_size, self.embedding_dim)
        c = torch.zeros(1, batch_size, self.embedding_dim)
