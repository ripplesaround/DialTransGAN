...
class CoT_G_trans(TranGenerator):
    def __init__(self, embedding_dim, vocab_size, max_seq_len, padding_idx, gpu=False):
        super(CoT_G_trans, self).__init__(embedding_dim, vocab_size, max_seq_len, padding_idx, gpu)
        self.name = 'cot_trans'
    ...
    def get_loss(self, input, rewards):
        ...
        @return:
        """
        log_pred = self.forward(input)  # (batch_size * seq_len) * vocab_size
